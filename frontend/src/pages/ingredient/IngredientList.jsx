...
const IngredientList = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(result.message || `Bahan ${formData.name} berhasil disimpan!`);
      setFormData({ name: "", stock: "", unit: "", minStock: "" });
  ...
  const handleDelete = async (id, name) => {
    if (
      ...
      try {
        ...
        alert(result.message || `Bahan ${name} berhasil dihapus.`);
      } catch (error) {
        alert(error.response?.data?.message || error.message || "Gagal hapus!");
      }
    }
  };
  ...
  return (
    <div className="ingredient-container">
      <div className="header-page">
        <h2 style={{ color: "#092379" }}>Bahan Baku</h2>
